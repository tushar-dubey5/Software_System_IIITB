...
int main() {
    ...
    if (pid1 == 0) { // Child process 1 (ls -l)
        // Set the write end of the pipe to standard output
        fcntl(pipefd[1], F_SETFD, FD_CLOEXEC); // Ensure the write end closes on exec
    ...
    if (pid2 == 0) { // Child process 2 (wc)
        // Set the read end of the pipe to standard input
        fcntl(pipefd[0], F_SETFD, FD_CLOEXEC); // Ensure the read end closes on exec
